...
Inverse is defined the same way but it also takes the inverse operation as an argument.

This formulation allows us to reuse these structures for addition and multiplication.
...
module FieldTheorems {F : Set} (f : Field F) where
  ...
  open Identity ·-1-identity
    renaming ( left-identity to ·-left-identity ; right-identity to ·-right-identity )
  open Inverse +-0-inverse
    renaming ( left-inverse to +-left-inverse ; right-inverse to +-right-inverse )
  open Inverse ·-1-inverse
    renaming ( left-inverse to ·-left-inverse ; right-inverse to ·-right-inverse )
...
\begin{code}
  ...
  ·-cancellation-equality : ∀ {x y : F} → x / y ≡ 1 → x ≡ y
  ·-cancellation-equality {x} {y} x/y≡1 =
    x ≡⟨ sym (·-right-identity x) ⟩
    x · 1 ≡⟨ cong (λ t → x · t) (sym (·-left-inverse y)) ⟩
    x · (y ⁻¹ · y) ≡⟨ ·-assoc x (y ⁻¹) y ⟩
    (x · y ⁻¹) · y ≡⟨ cong (λ t → t · y) x/y≡1 ⟩
    1 · y ≡⟨ ·-left-identity y ⟩
    y ∎
